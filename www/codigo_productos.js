...
var createStatement = "CREATE TABLE IF NOT EXISTS Products (id INTEGER PRIMARY KEY AUTOINCREMENT, product_name varchar(50), description varchar(150), price varchar(50), quantity varchar(50), category varchar(20), user_name varchar(50), email varchar(50), phone varchar(30), cellphone varchar(30), address varchar(150))";
 
var selectAllStatement = "SELECT * FROM Products";
 
var insertStatement = "INSERT INTO Products (product_name, description, price, quantity, category, user_name, email, phone, cellphone, address) VALUES (?,?,?,?,?,?,?,?,?,?)";
...
function insertRecord(user_name, category, email, phone, cellphone, address) // Get value from Input and insert record . Function Call when Save/Submit Button Click..
 
{
        ...
        if (product_nametemp == "" || descriptiontemp == "" || pricetemp == "" ) {
            ...
            db.transaction(function (tx) { tx.executeSql(insertStatement, [product_nametemp, descriptiontemp, pricetemp, quantitytemp, category, user_name, email, phone, cellphone, address ], loadAndReset, onError); });
        ...
        tx.executeSql(selectAllStatement, [], function (tx, result) {
         if( onloadDone == false)
         {
            ...
            for (var i = 0, item = null; i < dataset.length; i++) {
                ...
                var linkeditdelete = '<li class="item">' + item['id'] + '   ' + item['product_name'] + '   ' + item['category'] + '   ' + item['user_name'] + '   ' + '<a href="#" onclick="loadRecord(' + i + ');">Editar</a>' + '    ' +
